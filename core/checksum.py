...
def _open_binary(path):
    """Default opener: a plain warm binary read."""
    return open(path, "rb")


def compute_all(path: Path, include_xxh128=True, include_md5=False,
                progress_cb: Optional[Callable[[int], None]] = None,
                opener: Callable = _open_binary) -> dict:
    ...
    M14.2: ``opener`` injects how the file is opened for reading. Defaults to a
    warm binary ``open``. ``verify_local`` passes ``cold_read.cold_open`` so a
    post-copy destination read bypasses the page cache where the platform allows.
    """
    ...
    result = {}
    if h_xx:  result["xxh128"] = h_xx.hexdigest()
