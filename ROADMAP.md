...
**Remaining for DONE (integration, needs a settings remote):** (1) wire `ship_logs` to fire after each operation and on app launch; (2) a Settings field for the shared Drive remote base + the opt-out toggle; (3) surface `pending_status` (status line + 7-day banner) in the GUI; (4) one manual end-to-end against a junk Drive folder. These ride together with M9.3 / the Settings work since the remote base must be configured first. README "Org-wide activity log" section added.
...
**Remaining for DONE:** wire `append_activity(record_for(...))` into the completion path of each operation (offload/transfer/merge/verify) — local-only, low risk, but each call site needs its own record-construction + test. Rides with the M9.1 integration step above.
