...
### M7.2 CI on GitHub Actions — CODE COMPLETE 2026-06-12, pending first green CI run
...
**State (2026-06-12):** `.github/workflows/ci.yml` added — `macos-latest`, Python 3.11, installs `requirements.txt` + `requirements-dev.txt` + `pytest-cov`, runs the **full** suite (no GUI ignores) with `QT_QPA_PLATFORM=offscreen` so the pytest-qt tests render headlessly, prints `--cov=core --cov=utils` term-missing coverage, fails the job on any test failure. Triggers on push + PR to `main`. README gained the CI status badge. YAML validated. Committed locally (`8ab6977`). **Cannot be self-verified here:** the first actual Actions run is the acceptance check — it confirms (a) the GUI tests pass under `offscreen` on the runner and (b) the badge resolves. The Linux dev sandbox has no PyQt6 so the GUI tests can't be pre-run locally. **Remaining for DONE: push to main, confirm the workflow goes green (fix any GUI test that needs a real display rather than offscreen), and confirm the badge renders.** If specific GUI tests can't run headlessly, mark them `@pytest.mark.skipif` on offscreen with a note, rather than weakening the suite.
